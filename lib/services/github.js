...
function GitHub (cache) {
	...
	function consumePages (url, items) {
		...
		.spread(function (response, body) {
			...
			switch (response.statusCode) {
				...
				case 404 : {
					throw Boom.notFound("Failed to retrieve resource '" + url + "'.");
				}
				default : {
	...
	this.organization = function (name) {
		return consumePages(url("users", name, "repos"))
		.catch(function (error) {
			if (404 === error.output.statusCode) {
				throw Boom.notFound("Organization '" + name + "' does not exist.");
			}
			throw error;
		});
	};
	...
	this.project = function (owner, name) {
		return request(url("repos", owner, name))
		.spread(function (response, body) {
			switch (response.statusCode) {
				case 200: {
					return body;
